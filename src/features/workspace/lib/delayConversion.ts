/** Conversion between the AudioSync engine's measurements and the delay values MkvBatchMux stores.
 *  Two conventions disagree about both sign and unit, so every conversion goes through here. */
 ...
 *  Negated: the engine reports where the audio sits, but `--sync` and this app ask how much delay to add. */
 ...
 *  With drift, `delayMs` is measured at the midpoint but `--sync` applies from t=0; `delayAtStartMs` corrects for that. */
 ...
 *  Rejects likely cuts, implausible magnitudes, and low-confidence correlations — high confidence alone doesn't rule out a locked-on repeated phrase. */
...
/** The measured offset in the player convention (unrounded), shown next to the rounded field value. */
...
/** Map a 0-1 engine confidence onto the three bands the UI displays; matches AudioSyncMaster's bands. */
...
/** A delay expressed in video frames; null below half a frame, where there's nothing useful to say. */
 ...
 *  mkvmerge applies a stretch ratio literally to every timestamp, so 23.976 must be 24000/1001, not the decimal — building from decimals costs ~3.6ms/hr of avoidable drift. */
...
/** 23.976 and 24 fps are only 0.024 apart and take different ratios.
 *  Stays under half that gap so a tolerance never silently picks the wrong conversion. */
const FPS_TOLERANCE = 0.01;

/** How close a measured speed must sit to a standard conversion before it's named on the measurement alone.
 *  The narrowest real conversion (24 → 23.976) is 0.1% wide, so this stays well inside it. */
const RATIO_TOLERANCE = 0.0004;

/** The exact rational for a reported frame rate, or null if it's not standard (e.g. a codec's own frame rate like 31.25 for AC-3). */
...
/** Where a conversion's numbers came from, worst case last: `named` and `inferred` are exact, `measured` carries the measurement's error. */
...
export interface RateConversion {
  ...
  /** mkvmerge's linear-stretch factor (`--sync <tid>:<ms>,<num>/<den>`); `num/den = audioFps / videoFps`.
   *  Reciprocal of the engine's `correctionRatio` (an atempo value) — swapping them doubles the drift instead of removing it. */
  num: number;
  den: number;
  basis: RateConversionBasis;
}

/** The exact conversion between two standard frame rates; null if either isn't standard or they're equal. */
 ...
 *  24→23.976, 30→29.97, and 60→59.94 share the same 1001/1000 factor, so the video's own rate is needed to pick the names. */
 ...
 *  A fixed-denominator rounding of 1.001001 can land on 999/1000 instead of 1001/1000 — looks exact, isn't. */
...
/** What frame-rate change this audio needs, and the exact ratio that makes it; null when nothing trustworthy applies. */
export function rateConversionFor(
  ...
  // Nothing standard fits; apply the measured ratio directly.
  // The video's own rate still lets the implied audio rate be named.
...
/** The change a conversion makes, written as the user thinks about it (which rate to which); falls back to the bare factor when neither rate can be named. */
...
/** How the mismatch shows up while watching: percent off plus seconds of drift per hour — a percentage alone is too abstract to act on. */
...
/** Why a stored measurement's delay was not staged for Apply, or null if it was.
 *  Derived from the stored record, not the live result, so a row can still explain itself after a reload. */
